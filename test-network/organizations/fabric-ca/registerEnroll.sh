...
function createOrg1 {
    ...
    # mkdir -p ( -p flag used to create the sub-directories of a directory )
    ...
    # set -x enables a mode of the shell where all stop commands are printed to the terminal
    { set +x; } 2>/dev/null
    # *--------------------------------* #
    # NodeOUs ( Node organizational unit's )
    echo 'NodeOUs:
    Enable: true
    ClientOUIdentifier:
