...
# queryInstalled command used to query the installed chaincode from the PEER ORG
queryInstalled() {
    ORG=$1
    setGlobals $ORG
    set -x
...
# approveForMyOrg approve a chaincode definition for your organization.
approveForMyOrg() {
    ORG=$1
    setGlobals $ORG
    set -x
...
# Used to check chaincode definition is ready to be committed.
# It also outputs which organizations have approved the chaincode definition.
